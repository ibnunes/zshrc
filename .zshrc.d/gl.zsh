...
runvk() {
    ...
    if [ ! -z $(type 'runcpp' | grep 'function') ]; then
        runcpp -c "$FILENAME.cpp"
        runcpp "$FILENAME.o" -o "$FILENAME" -lvulkan -lglfw3 -lX11 -lXxf86vm -lXrandr -lpthread -lXi -ldl -lXinerama -lXcursor
    else
        g++ -std=c++20 -c "$FILENAME.cpp"
        g++ "$FILENAME.o" -o "$FILENAME" -lvulkan -lglfw3 -lX11 -lXxf86vm -lXrandr -lpthread -lXi -ldl -lXinerama -lXcursor
    fi
}
